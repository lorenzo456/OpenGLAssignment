...
void startup() {
	...
	Shader vertexShader = Shader("vs_model.glsl", GL_VERTEX_SHADER);
	glAttachShader(program, vertexShader.ReturnShader());
	Shader fragmentShaderTextures = Shader("fs_textures.glsl", GL_FRAGMENT_SHADER);
	glAttachShader(program, fragmentShaderTextures.ReturnShader());
...
void render(GLfloat currentTime) {
	...
	// Clear colour buffer
	glm::vec4 backgroundColor = glm::vec4(0.8f, 0.2f, 0.2f, 1.0f); glClearBufferfv(GL_COLOR, 0, &backgroundColor[0]);
	...
	glUseProgram(program);
	testObject.Update(program, time, viewMatrix, proj_matrix);
	
	glUseProgram(waveShaderProgram);
	testObject1.Update(waveShaderProgram, time, viewMatrix, proj_matrix);
}
